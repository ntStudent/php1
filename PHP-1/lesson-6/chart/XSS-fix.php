...
//    strip_tags - вырезает все html теги
...
//    htmlspecialchars - преобразует все html-атрибуты в спецсимволы
    ...
    if ($name != '' && $text != ''){
        $query = $db->prepare("INSERT INTO comments2 SET name='$name', text='$text'");//подготовка занесение данных в таблицу "comments2" в базе данных "mySite"
        $query->execute();//выполнение занесения данных в таблицу

        header("Location: XSS-fix.php");//возвращаемся на нашу страницу методом "GET"
        exit();
    }
}

$query = $db->prepare("SELECT * FROM comments2 ORDER BY dt DESC ");//выбираем все данные в таблице "comments2" с сортировкой по дате "ORDER BY dt DESC" по убыванию новые вверху для вывода данных
